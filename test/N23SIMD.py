...
# ---------------------------------------------------------------------------
# Zero-shot evaluation of SIMD model on NREM2/3 sleep-EEG data
#
# Workflow
#   1. Load a pretrained SIMD model checkpoint.
...
if __name__ == "__main__":
    ...
    for sid in subject_ids:
        ...
        for sleep_eeg, labels in dataset:
            # SIMD expects three domains; supply random tensors for two of them
            rand_img   = tf.random.normal((sleep_eeg.shape[0], 80, 55))
            rand_audio = tf.random.normal((sleep_eeg.shape[0], 80, 55))
    ...
    # ----------------------------- save -------------------------------------
    save_path = os.path.join(basepath, 'results', 'N23SIMD_Decode_Result.pickle')
