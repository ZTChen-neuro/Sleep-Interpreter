...
# ---------------------------------------------------------------------------
# Zero-shot evaluation of SIMD model on REM sleep-EEG data
#
# Workflow
...
def load_test_data(path, batch_size):
    """
    Load pre-serialised test data for a subject's REM data.
...
if __name__ == "__main__":
    ...
    model_item       = SIMD()
    checkpoint_path  = os.path.join(checkpath, "REM_SIMD.ckpt")
    ...
    for sid in subject_ids:
        ...
        data_file  = sorted(
            ...
            if os.path.basename(p).startswith("REM")
        )[0]
