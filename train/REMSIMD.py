...
if __name__ == "__main__":
    # Ensure project-level packages (utils, model, …) are import‑able
    sys.path.insert(0, os.path.join(os.getcwd(), os.pardir))
    import utils
    from model.REMSIMD.multi_encoder import multi_encoder
    from model.REMSIMD import SIMD

# ---------------------------------------------------------------------------
# REM‑phase multidomain decoding script (REM‑SIMD ensemble)
# ---------------------------------------------------------------------------
#   • Reads TFRecord examples that contain synchronized image, audio, EEG, and
#     metadata features for *REM* sleep epochs.
...
def _parse_function(example_proto, data_type):
    ...
    Each record contains serialized tensors with the following keys:
        • category_name         – one‑hot (15,) describing REM micro‑state
        • feature_awake_image   – 15×55×80 preprocessed awake image EEG data (wake context)
        • feature_awake_audio   – 15×55×80 preprocessed awake audio data (wake context)
        • feature_Sleep_EEG     – 55×200 preprocessed sleep EEG data (REM epoch)
    ...
    awake_image   = tf.io.parse_tensor(parsed['feature_awake_image'], tf.float32)
    awake_audio   = tf.io.parse_tensor(parsed['feature_awake_audio'], tf.float32)
    ...
    awake_image = tf.transpose(tf.reshape(awake_image, [15, 55, 80]), [0, 2, 1])
    awake_audio = tf.transpose(tf.reshape(awake_audio, [15, 55, 80]), [0, 2, 1])
    rand_idx    = tf.random.shuffle(tf.range(15))[:1]  # pick 1 frame at random
    awake_image = tf.gather(awake_image, rand_idx, axis=0)
    awake_audio = tf.gather(awake_audio, rand_idx, axis=0)
    ...
    return awake_image, awake_audio, sleep_eeg, category_name
...
if __name__ == "__main__":
    ...
    cycle_length   = 1
    ensemble_models  = 5
    data_type      = 'awake and REM'
    epochs         = 100

    # -------------------- Instantiate ensemble --------------------
    models = [multi_encoder() for _ in range(ensemble_models)]

    # -------------------- TFRecord glob (REM subset) --------------------
    rem_dir   = os.path.join(datapath, 'REM')
    rem_files = sorted(os.path.join(rem_dir, f) for f in os.listdir(rem_dir) if f.startswith('whole'))
    ...
    for epoch in range(epochs):
        ...
        for i, m in enumerate(models):
            ckpt = os.path.join(result_checkpoint, f'REM_SIMD_epoch{epoch:02d}_model{i:02d}.ckpt')
            m.save_weights(ckpt)
    ...
    for i in range(ensemble_models):
        ckpt_last = os.path.join(result_checkpoint, f'REM_SIMD_epoch{epochs-1:02d}_model{i:02d}.ckpt')
        ensemble.individual_models[i].load_weights(ckpt_last)

    ensemble_ckpt = os.path.join(result_checkpoint, 'REM_SIMD_model.ckpt')
