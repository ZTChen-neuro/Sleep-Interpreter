...
# -----------------------------------------------------------
# SO-event zero-shot decoding with SIMD model
#
# This script:
#   1. Loads a pre-trained SIMD model checkpoint.
#   2. Iterates over a test set of sleep-EEG segments for 12 subjects.
#   3. Computes zero-shot accuracy for eight SO-event subclasses.
#   4. Saves a dictionary of results to disk as a pickle.
#
# -----------------------------------------------------------
...
if __name__ == "__main__":
    ...
    checkpoint_path  = os.path.join(checkpath, "N2N3_SIMD.ckpt")
    ...
    save_path = os.path.join(basepath, 'results', 'N23SIMD_SOevent_Decode_Result.pickle')
