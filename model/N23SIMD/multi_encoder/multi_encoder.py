...
class multi_encoder(K.Model):
    ...
    def __init__(self, mode='normal', finetune=False, **kwargs):
        """
        ...
        # Using whole model or smaller model for retraining
        self.mode = mode
    ...
    def _init_trainable(self):
        ...
        if not self.finetune:
            self.image_emb_input = SubjectBlock([204, 256, 256], self.mode)
            self.audio_emb_input = SubjectBlock([204, 256, 256], self.mode)
            self.sleep_emb_input = SleepSubjectBlock([204, 256, 256], self.mode)
        else:
            self.image_emb_input = SubjectBlock_ft([204, 256, 256], self.mode)
            self.audio_emb_input = SubjectBlock_ft([204, 256, 256], self.mode)
            self.sleep_emb_input = SleepSubjectBlock_ft([204, 256, 256], self.mode)
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        awake_image_emb = self.image_emb_input(image_data)
        awake_image_emb = self.image_encoder(awake_image_emb)
        
        awake_audio_emb = self.audio_emb_input(audio_data)
        ...
        loss = ((5 * loss_sleep + loss_image + 1 * loss_audio)/7) + \
            ((0.5 * awake_loss_contra + 2.0 * sleep_loss_contra + 0.5 * image_loss_contra) / 3.)
