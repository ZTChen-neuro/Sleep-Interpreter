...
class lasso_glm:
    ...
    def fit(self, X, y):
        ...
        Returns:
            ...
            acc_test: (seq_len,) - The test accuracy of ecah asso classifier trained at each time point.
        """
        # Initialize `X_train` & `X_test1` & `X_test` from `X`, `y_train` & `y_test1` & `y_test` from `y`.
        X_train, X_test1, X_test = X; y_train, y_test1, y_test = y
        ...
        try:
            ...
            acc_test = np.array(accuracy[1,:], dtype=np.float32)
        except matlab.engine.MatlabExecutionError as e:
            raise ValueError("ERROR: Get matlab.engine.MatlabExecutionError {}.".format(e))
        # Close the matlab engine.
        mat_eng.quit()
        # Return the final `acc_test1` & `acc_test`.
        return acc_test1, acc_test
...
if __name__ == "__main__":
    ...
    # Construct train-set & test1-set & test-set using loo.
    ...
    train_idxs = list(set(range(y.shape[0])) - set(test1_idxs) - set(test_idxs))
    assert len(train_idxs) + len(test1_idxs) + len(test_idxs) == y.shape[0]
    # Use `train_idxs` & `test1_idxs` & `test_idxs` to get train-set & test1-set & test-set.
    ...
    accuracy_test1, accuracy_test = lasso_glm_inst.fit((X_train, X_test1, X_test), (y_train, y_test1, y_test))
    print((
        "The accuracy of test1-set is {:.2f}%, the accuracy of test-set is {:.2f}%."
    ).format(np.max(accuracy_test1)*100., np.max(accuracy_test)*100.))
