...
class stager(K.Model):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        # label - (batch_size,)
        X = inputs[0]; true_label = inputs[1]; label = inputs[2]
        # value - corresponding values ​​for calculating MSE loss in different sleep stages
        value = tf.constant([0.,1.,2.,3.], dtype = tf.float32)
        values = tf.tile(tf.expand_dims(value, 0), [X.shape[0], 1])
        ...
        y_pred_mse = tf.cast(tf.argmax(output, axis = 1), dtype = tf.float32)
        # loss_mse - tf.float32
        loss_mse = tf.reduce_mean(tf.square(y_pred_mse - label), axis = 0)
        
        # Calculate the binary cross entropy loss.
        # loss_bce - tf.float32
        loss_bce = tf.reduce_mean(self._loss_bce(output, true_label, self.weight))
        # Calculate the final `loss`.
        loss = 1. * loss_bce + 1. * loss_mse

        return loss,output

    # def _loss_bce func
    @utils.model.tf_scope
    def _loss_bce(self, value, target, weight):
        """
        ...
        :param weight: (4,) - Weight preference of the four different stages.
        ...
        # loss - (batch_size,)
        self.weight = tf.Variable(weight, trainable=False, name="w")    
        weighted_losses = tf.nn.weighted_cross_entropy_with_logits(labels=target, logits=value, pos_weight=self.weight) if type(value) is not list else [tf.nn.softmax_cross_entropy_with_logits(labels=target[i],logits=value[i] , pos_weight=self.weight) for i in range(len(value))]
        weighted_losses = tf.convert_to_tensor(weighted_losses, dtype = tf.float32)
        weighted_losses = tf.reduce_mean(weighted_losses, axis =1)
        # return loss
