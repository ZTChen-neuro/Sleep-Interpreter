...
"""
REM-to-Awake transfer-learning benchmark using a LASSO-regularised GLM.

Pipeline
--------
1.  Load awake EEG (image or audio paradigm) as training data.  
2.  For each subject and every 2-s data sequence in the REM data for training,
...
def load_train_data(path, data_type, time_point):
    """
    Extract an *8-s window* from the REM sleep stage.
...
def train(params_):
    ...
    for index, index_number in enumerate(index_numbers):
        ...
        for time_point in start_time_point_list:
            ...
            train_dataset = load_train_data(path_data, 'REM', time_point)
        ...
        data_save_path = os.path.join(resultpath, "lasso_REM2awake.pickle")
