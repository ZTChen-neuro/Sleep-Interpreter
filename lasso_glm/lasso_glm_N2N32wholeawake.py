...
"""
NREM2/3 → awake transfer-learning benchmark  
 • Trains a LASSO-regularised GLM on every single time point (200) of NREM2/3 sleep EEG.  
 • Evaluates on 80-sample points from awake image and awake audio sleep.  
...
def train(params_):
    ...
    for index_number in index_numbers:
        ...
        for data_type in type_list:
            ...
            Result_dict[index_number][data_type]['awake image'] = []
            Result_dict[index_number][data_type]['awake audio'] = []
            ...
            if len(train_dataset) == 1:
                NREM_check = True
                Result_dict[index_number][data_type]['awake image'] = [0] * 80
                Result_dict[index_number][data_type]['awake audio'] = [0] * 80
                continue
            ...
            while run_idx < 1:
                ...
                test_dataset1 = load_data(path_data, 'awake image')
                test_dataset2 = load_data(path_data, 'awake audio')
            ...
            Result_dict[index_number][data_type]['awake image'] = accuracies_test1*100
            Result_dict[index_number][data_type]['awake audio'] = accuracies_test2*100
            
        data_save_path = os.path.join(resultpath, "lasso_NREM2awake_temporal.pickle")
