...
"""
Awake-to-Sleep transfer-learning benchmark using a LASSO-regularised GLM.

Pipeline
--------
1.  Load awake EEG (image or audio paradigm) as training data.  
2.  For each subject and every 2-s data sequence,
    slice an 800-ms window from the same night’s N2/N3 or REM data
    to build test sets.  
...
def load_train_data(path, data_type):
    """
    Load *awake* EEG training data from a pkl file.
...
def load_test_data(path, data_type, time_point):
    """
    Extract an *8-s window* from the specified sleep stage (N2N3 or REM).
...
def train(params_):
    ...
    recordpath = os.path.join(basepath, "data", 'SI Main Train')
    recordpath2 = os.path.join(basepath, "data", 'SI Main Test')
    resultpath = os.path.join(basepath, 'results')
    type_list  = ['awake_image', 'awake_audio']        # two awake paradigms
    ...
    Result_dict = {}
    for index, subj in enumerate(index_numbers):
        Result_dict[subj] = {}
        NREM_check, REM_check = False, False
        for data_type in type_list:
            Result_dict[subj][data_type] = {}

            # Single combined pickle per subject
            ...
            train_dataset = load_train_data(path_data, data_type)
            for time_point in start_time_point_list:
                Result_dict[subj][data_type][time_point] = {'N2N3': [], 'REM': []}
                
                # ------------------------------------------------------------
                # Build test1 (N2N3) and test2 (REM) datasets
                # ------------------------------------------------------------

                test_dataset1 = load_test_data(path_data, 'N2N3', time_point)
                test_dataset2 = load_test_data(path_data, 'REM', time_point)
                if len(test_dataset2) == 1:
                    REM_check = True
                    test_dataset2 = test_dataset1
                    if len(test_dataset1) == 1:
                        print(subj)
                        assert 1==2
                elif len(test_dataset1) == 1:
                    NREM_check = True
                ...
                if NREM_check:
                    Result_dict[subj][data_type][time_point]['N2N3'] = [0] * 80
                else:
                    Result_dict[subj][data_type][time_point]['N2N3'].append(avg_accuracy_test1*100)
                if REM_check:
                    Result_dict[subj][data_type][time_point]['REM'] = [0] * 80
                else:
                    Result_dict[subj][data_type][time_point]['REM'].append(avg_accuracy_test2*100)

        # Persist per-subject results after each outer loop iteration
        save_path = os.path.join(resultpath, "lasso_awake2sleep.pickle")
