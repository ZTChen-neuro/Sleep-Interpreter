...
# ---------------------------------------------------------------------------
# Multi-domain NREM Sleep Decoder – Fine-tuning Script
# ---------------------------------------------------------------------------
# This script fine-tunes a *multi-encoder* model that combines image, audio
# and EEG inputs to classify NREM2/3-stage sleep data.  
# ---------------------------------------------------------------------------
...
def load_data(path, batch_size, data_type):
    ...
    for sample in dataset:
        ...
        image_data.append(sample['awake_image'])
        audio_data.append(sample['awake_audio'])
    ...
    dataset_tf = tf.data.Dataset.from_tensor_slices(
        (awake_img, awake_aud, sleep_eeg, category, audio)
    )
...
if __name__ == "__main__":
    ...
    # List of test subjects / folds
    index_numbers = ['%02d' % i for i in range(1, 13)]
